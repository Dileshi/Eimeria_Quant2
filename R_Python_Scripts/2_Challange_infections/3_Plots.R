...
challenge.DNA%>%
  ...
  labs(tag= "a", shape= "qPCR results of Secondary Infection")+
...
x <- stats.test
x$groups<- NULL
#write.csv(x, "Tables/Secondary_Infections/OPG_DPI_Challenged_Comparison.csv")

challenge.DNA%>%
  dplyr::select(EH_ID, dpi, OPG)%>%
  ggplot(aes(factor(dpi), OPG+1, colour = factor(dpi)))+ #remove colour=dpi to get black outline boxplot
  geom_boxplot(fill = "white")+
...
challenge.DNA%>%
  ...
  ggplot(aes(factor(dpi), weightloss, colour =factor(dpi)))+ #remove colour=dpi to get black outline boxplot #replace fill with colour for no balck outline and coloured boxplot
  geom_boxplot(fill = "white")+
...
pdf(file = "Figures/Figure_3A_test_3.pdf", width = 10, height = 15)
grid.arrange(A,B,C)
dev.off()
